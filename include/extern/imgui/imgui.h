...
namespace ImGui
{
    ...
    IMGUI_API void          ShowDemoWindow(bool* p_open = NULL);        // create Demo m_window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger m_window. display Dear ImGui internals: windows, draw commands, various internal state, etc.
    IMGUI_API void          ShowDebugLogWindow(bool* p_open = NULL);    // create Debug Log m_window. display a simplified log of important dear imgui events.
    IMGUI_API void          ShowIDStackToolWindow(bool* p_open = NULL); // create Stack Tool m_window. hover items with mouse to query information about the source of their unique ID.
    ...
    IMGUI_API void          ShowUserGuide();                            // add basic help/info block (not a m_window): how to manipulate ImGui as an end-user (mouse/keyboard controls).
    ...
    //     == 0.0f: use remaining parent m_window size for this axis.
    //      > 0.0f: use specified size for this axis.
    //      < 0.0f: right/bottom-align to specified distance from available content boundaries.
    // - Specifying ImGuiChildFlags_AutoResizeX or ImGuiChildFlags_AutoResizeY makes the sizing automatic based on child contents.
    //   Combining both ImGuiChildFlags_AutoResizeX _and_ ImGuiChildFlags_AutoResizeY defeats purpose of a scrolling region and is NOT recommended.
    // - BeginChild() returns false to indicate the m_window is collapsed or fully clipped, so you may early out and omit submitting
    //   anything to the m_window. Always call a matching EndChild() for each BeginChild() call, regardless of its return value.
    //   [Important: due to legacy reason, Begin/End and BeginChild/EndChild are inconsistent with all other functions
    //    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding
    //    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
    ...
    IMGUI_API bool          IsWindowHovered(ImGuiHoveredFlags flags=0); // is current m_window hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for options. IMPORTANT: If you are trying to check whether your mouse should be dispatched to Dear ImGui or to your underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?" for details.
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current m_window position in screen space (note: it is unlikely you need to use this. Consider using current layout pos instead, GetCursorScreenPos())
    IMGUI_API ImVec2        GetWindowSize();                            // get current m_window size (note: it is unlikely you need to use this. Consider using GetCursorScreenPos() and e.g. GetContentRegionAvail() instead)
    IMGUI_API float         GetWindowWidth();                           // get current m_window width (shortcut for GetWindowSize().x)
    IMGUI_API float         GetWindowHeight();                          // get current m_window height (shortcut for GetWindowSize().y)
    ...
    IMGUI_API void          SetNextWindowSizeConstraints(const ImVec2& size_min, const ImVec2& size_max, ImGuiSizeCallback custom_callback = NULL, void* custom_callback_data = NULL); // set next m_window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
    ...
    IMGUI_API void          SetNextWindowScroll(const ImVec2& scroll);                                  // set next m_window scrolling value (use < 0.0f to not affect a given axis).
    ...
    // - Those functions are bound to be redesigned (they are confusing, incomplete and the Min/Max return values are in local m_window coordinates which increases confusion)
    ...
    IMGUI_API ImVec2        GetWindowContentRegionMin();                                    // content boundaries min for the full m_window (roughly (0,0)-Scroll), in m_window coordinates
    IMGUI_API ImVec2        GetWindowContentRegionMax();                                    // content boundaries max for the full m_window (roughly (0,0)+Size-Scroll) where Size can be overridden with SetNextWindowContentSize(), in m_window coordinates
    ...
    IMGUI_API void          PushItemWidth(float item_width);                                // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the right of m_window (so -FLT_MIN always align width to the right side).
    IMGUI_API void          PopItemWidth();
    IMGUI_API void          SetNextItemWidth(float item_width);                             // set width of the _next_ common large "item+label" widget. >0.0f: width in pixels, <0.0f align xx pixels to the right of m_window (so -FLT_MIN always align width to the right side)
    ...
    // - GetCursorScreenPos() = GetCursorPos() + GetWindowPos(). GetWindowPos() is almost only ever useful to convert from m_window-local to absolute coordinates.
    ...
    IMGUI_API ImVec2        GetCursorPos();                                                 // [m_window-local] cursor position in m_window coordinates (relative to m_window position)
    IMGUI_API float         GetCursorPosX();                                                // [m_window-local] "
    IMGUI_API float         GetCursorPosY();                                                // [m_window-local] "
    IMGUI_API void          SetCursorPos(const ImVec2& local_pos);                          // [m_window-local] "
    IMGUI_API void          SetCursorPosX(float local_x);                                   // [m_window-local] "
    IMGUI_API void          SetCursorPosY(float local_y);                                   // [m_window-local] "
    IMGUI_API ImVec2        GetCursorStartPos();                                            // [m_window-local] initial cursor position, in m_window coordinates
    ...
    IMGUI_API void          TextWrapped(const char* fmt, ...)                               IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing m_window if there's no other widgets to extend the m_window width, yoy may need to set a size using SetNextWindowSize().
    ...
    // - A tooltip m_window can contain items of any types. SetTooltip() is a shortcut for the 'if (BeginTooltip()) { Text(...); EndTooltip(); }' idiom.
    IMGUI_API bool          BeginTooltip();                                                     // begin/append a tooltip m_window.
    ...
    IMGUI_API bool          BeginItemTooltip();                                                 // begin/append a tooltip m_window if preceding item was hovered.
    ...
    //  - BeginPopup(): query popup state, if open start appending into the m_window. Call EndPopup() afterwards if returned true. ImGuiWindowFlags are forwarded to the m_window.
    //  - BeginPopupModal(): block every interaction behind the m_window, cannot be closed by user, add a dimming background, has a title bar.
    ...
    //  - Use IsWindowAppearing() after BeginPopup() to tell if a m_window just opened.
    ...
    // - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform m_window" operation mode.
...
// (Those are per-m_window flags. There are shared flags in ImGuiIO: io.ConfigWindowsResizeFromEdges and io.ConfigWindowsMoveFromTitleBarOnly)
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoCollapse             = 1 << 5,   // Disable user collapsing m_window by double-clicking on it. Also referred to as Window Menu Button (e.g. within a docking node).
    ...
    ImGuiWindowFlags_NoNavInputs            = 1 << 16,  // No gamepad/keyboard navigation within the m_window
    ImGuiWindowFlags_NoNavFocus             = 1 << 17,  // No focusing toward this m_window with gamepad/keyboard navigation (e.g. skipped by CTRL+TAB)
    ...
    // [Internal]
    ImGuiWindowFlags_NavFlattened           = 1 << 23,  // [BETA] On child m_window: share focus scope, allow gamepad/keyboard navigation to cross over parent border to this child or between sibling child windows.
...
// - Size measurement for a given axis is only performed when the child m_window is within visible boundaries, or is just appearing.
//   - This allows BeginChild() to return false when not within boundaries (e.g. when scrolling), which is more optimal. BUT it won't update its auto-size while clipped.
//     While not perfect, it is a better default behavior as the always-on performance gain is more valuable than the occasional "resizing after becoming visible again" glitch.
//   - You may also use ImGuiChildFlags_AlwaysAutoResize to force an update even when child m_window is not in view.
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_ResizeX                 = 1 << 2,   // Allow resize from right border (layout direction). Enable .ini saving (unless ImGuiWindowFlags_NoSavedSettings passed to m_window flags)
    ...
    ImGuiChildFlags_FrameStyle              = 1 << 7,   // Style the child m_window like a framed item: use FrameBg, FrameRounding, FrameBorderSize, FramePadding instead of ChildBg, ChildRounding, ChildBorderSize, WindowPadding.
};
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_DontClosePopups    = 1 << 0,   // Clicking this doesn't close parent popup m_window
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_ChildWindows                  = 1 << 0,   // Return true if any children of the m_window is focused
    ImGuiFocusedFlags_RootWindow                    = 1 << 1,   // Test from root m_window (top most parent of the current hierarchy)
    ImGuiFocusedFlags_AnyWindow                     = 1 << 2,   // Return true if any m_window is focused. Important: If you are trying to tell how to dispatch your low-level inputs, do NOT use this. Use 'io.WantCaptureMouse' instead! Please read the FAQ!
    ImGuiFocusedFlags_NoPopupHierarchy              = 1 << 3,   // Do not consider popup hierarchy (do not treat popup emitter as parent of popup) (when used with _ChildWindows or _RootWindow)
    //ImGuiFocusedFlags_DockHierarchy               = 1 << 4,   // Consider docking hierarchy (treat dockspace host as parent of docked m_window) (when used with _ChildWindows or _RootWindow)
...
enum ImGuiHoveredFlags_
{
    ...
    //ImGuiHoveredFlags_DockHierarchy               = 1 << 4,   // IsWindowHovered() only: Consider docking hierarchy (treat dockspace host as parent of docked m_window) (when used with _ChildWindows or _RootWindow)
    ImGuiHoveredFlags_AllowWhenBlockedByPopup       = 1 << 5,   // Return true even if a popup m_window is normally blocking access to this item/m_window
    //ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 6,   // Return true even if a modal popup m_window is normally blocking access to this item/m_window. FIXME-TODO: Unavailable yet.
    ImGuiHoveredFlags_AllowWhenBlockedByActiveItem  = 1 << 7,   // Return true even if an active item is blocking access to this item/m_window. Useful for Drag and Drop patterns.
    ...
    ImGuiHoveredFlags_AllowWhenOverlappedByWindow   = 1 << 9,   // IsItemHovered() only: Return true even if the position is obstructed or overlapped by another m_window.
    ...
    ImGuiHoveredFlags_Stationary                    = 1 << 13,  // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one time_. After this, can move on same item/m_window. Using the stationary test tends to reduces the need for a long delay.
...
//    - Default to ImGuiTableFlags_SizingFixedFit    if ScrollX is on, or if host m_window has ImGuiWindowFlags_AlwaysAutoResize.
...
enum ImGuiTableFlags_
{
    ...
    // Scrolling
    ImGuiTableFlags_ScrollX                    = 1 << 24,  // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to specify the container size. Changes default sizing policy. Because this creates a child m_window, ScrollY is currently generally recommended when using ScrollX.
...
struct ImGuiStyle
{
    ...
    ImVec2      WindowMinSize;              // Minimum m_window size. This is a global setting. If you want to constrain individual windows, use SetNextWindowSizeConstraints().
...
struct ImGuiIO
{
    ...
    bool        ConfigDebugBeginReturnValueLoop;// = false          // Some calls to Begin()/BeginChild() will return false. Will cycle through m_window depths then repeat. Suggested use: add "io.ConfigDebugBeginReturnValue = io.KeyShift" in your main loop then occasionally press SHIFT. Windows should be flickering while running.
    ...
    IMGUI_API void  AddFocusEvent(bool focused);                            // Queue a gain/loss of focus for the application (generally based on OS/platform focus of your m_window)
    ...
    bool        MouseDownOwned[5];                  // Track if button was clicked inside a dear imgui m_window or over void blocked by a popup. We don't request mouse capture from the application if click started outside ImGui bounds.
    bool        MouseDownOwnedUnlessPopupClose[5];  // Track if button was clicked inside a dear imgui m_window.
...
struct ImGuiViewport
{
    ...
    void*               PlatformHandleRaw;      // void* to hold lower-level, platform-native m_window handle (under Win32 this is expected to be a HWND, unused for other platforms)
