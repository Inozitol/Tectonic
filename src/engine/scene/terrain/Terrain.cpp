...
void Terrain::generateFlat(uint32_t X, uint32_t Z) {
    dimX = X;
    dimY = Z;
...
void Terrain::createMaterial() {
    ...
    std::vector<DescriptorAllocatorDynamic::PoolSizeRatio> sizes = {{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 3}, {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 3}, {VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1}};

    descriptorPool.initPool(1, sizes);
    materialBuffer = VktBuffers::create(sizeof(VktTypes::GLTFMetallicRoughness::MaterialConstants) * 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
...
void Terrain::generateFlatPlane() {
    ...
    if((dimX - 1) % (patchSize - 1) != 0) {
        ...
        LOG(LOG_WARNING, "Terrain X dimension size of " << dimX << " on patch size of " << patchSize << " may result in issues. " << "Recommended size is " << nearestSize);
    ...
    if((dimY - 1) % (patchSize - 1) != 0) {
        ...
        LOG(LOG_WARNING, "Terrain Y dimension size of " << dimY << " on patch size of " << patchSize << " may result in issues. " << "Recommending size is " << nearestSize);
    ...
    for(int32_t y = 0; y < dimY; y++) {
        for(int32_t x = 0; x < dimX; x++) {
            Vertex_t vertex;
            vertex.position = {(static_cast<float>(x) - static_cast<float>(dimX) / 2) * worldScale, 0.0f, (static_cast<float>(y) - static_cast<float>(dimY) / 2) * worldScale};
            vertex.normal = {0.0f, 0.0f, 0.0f};
            vertex.uvX = x;
            vertex.uvY = y;
            vertices.at(hxy2i(x, y)) = vertex;
        }
    }

    patchManager.init(this);
    ...
    for(int32_t y = 0; y < dimY; y++) {
        for(int32_t x = 0; x < dimX; x++) {
            ...
            if(x % (patchSize-1) == 0 && y % (patchSize-1) == 0) {
                ...
                // Second point is one unit above. The infinity shader will project it to infinity.
                debugGridLines.vertices.emplace_back(VktTypes::GPU::Vertex<VktTypes::GPU::VertexType::POINT>{
                        .position = {vertex.position.x, vertex.position.y + 1.0f, vertex.position.z},
                        .color = {1.0f, 0.0f, 0.0f}
                });
            }
        }
...
void Terrain::clear() {
    LOG(LOG_INFO, "Clearing terrain resources");
...
void Terrain::setMaxLOD(uint32_t mLOD) {
    ...
    LOG(LOG_INFO, "Using " << mLOD << " LOD levels, with patch size of " << patchSize << " verticies.");
}
            ...
            for(uint8_t r = 0; r < RIGHT; r++) {
                for(uint8_t t = 0; t < TOP; t++) {
                    for(uint8_t b = 0; b < BOTTOM; b++) {
                        ...
                        LOG(LOG_DEBUG, "Created " << lodInfo.at(lod).info[l][r][t][b].count << " indices for LOD " << lod << " patch variant " << static_cast<uint32_t>(l) << ' ' << static_cast<uint32_t>(r) << ' ' << static_cast<uint32_t>(t) << ' ' << static_cast<uint32_t>(b));
                    }
                }
            }
        }
...
void Terrain::createFan(uint32_t x, uint32_t y, uint32_t lodCore, uint32_t lodLeft, uint32_t lodRight, uint32_t lodTop, uint32_t lodBottom) {
    const uint32_t stepLeft = Utils::binPow(lodLeft);
    const uint32_t stepRight = Utils::binPow(lodRight);
    const uint32_t stepTop = Utils::binPow(lodTop);
    const uint32_t stepBottom = Utils::binPow(lodBottom);
    const uint32_t stepCenter = Utils::binPow(lodCore);

    uint32_t iCnt = hxy2i(x + stepCenter, y + stepCenter);

    uint32_t indexTemp1 = hxy2i(x, y);
    uint32_t indexTemp2 = hxy2i(x, y + stepLeft);
...
void Terrain::connectCamera(Camera &camera) {
    ...
    camera.sig_position.connect(patchManager.slt_cameraPosition);
    patchManager.cameraPatch = patchManager.searchPatchBound(camera.position.x, camera.position.z).value();
    patchManager.sig_patchBoundsChange.connect(lodManager.slt_activePatch);
    patchManager.sig_patchBoundsChange.emit(patchManager.cameraPatch.xCoord, patchManager.cameraPatch.yCoord);
}
...
bool Terrain::isPatchInsideFrustum(uint32_t x, uint32_t y) {
    ...
    return frustumCulling.isPointInside(p00_low) || frustumCulling.isPointInside(p01_low) || frustumCulling.isPointInside(p10_low) || frustumCulling.isPointInside(p11_low) || frustumCulling.isPointInside(p00_high) || frustumCulling.isPointInside(p01_high) || frustumCulling.isPointInside(p10_high) || frustumCulling.isPointInside(p11_high);
